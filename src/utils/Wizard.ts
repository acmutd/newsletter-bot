    ...
    private setConfigurations(): WizardConfigurations {
        return {
            ...
            responses: {
                ...
                time:
                    "⏰ You ran out of time (+<time> seconds). Ending wizard...",
        ...
        if (this.wizard.additions) {
            ...
            messageEmbedOptionKeys.forEach((key) => {
                if (
                    ...
                    details[key] = ((additions[key] as string) +
                        details[key]) as any;
                }
            });
        }
                ...
                do {
                    ...
                    try {
                        var response = await this.wizard.message.channel.awaitMessages(
                            (m) =>
                                m.author.id === this.wizard.message.author.id,
                            {
                                max: 1,
                        ...
                        return {
                            status: WizardNodeResponseStatus.INCOMPLETE,
                            item: false,
                        };
                    }
                    ...
                    wizardNode.delete();
                    failed = !res;
                    details.color = defaultColor
                        ? defaultColor
                        : "NOT_QUITE_BLACK";
                } while (!res);
                if (res != this.wizard.configs.commands.doneLoop) {
                    item.push(res);
                    const cbResult: WizardNodeLoopCBResponse = await this.options.loopedCB(
                        item
                    );
                    if (cbResult) {
...
export class GraphicWizardNode extends WizardNode {
    ...
    async validationCB(response: Message) {
        var isMedia = await CheckerUtils.isMediaURL(
